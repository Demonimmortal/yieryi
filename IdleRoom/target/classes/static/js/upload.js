...
var uploader = new plupload.Uploader({
	...
	init: {
		PostInit: function() {
			...
			$('#submitgoods').on('click',function() {
				if(upconfig.filesNum<=0){
					...
					if(goods.checkSubForm()){
						upPic.addLoading();
						$('#uppic-progress-now').addClass('active');
		...
		FilesAdded: function(up, files) {
			upPic.checkPic();
			upconfig.count=0;
			upconfig.filesNum = uploader.files.length;
			plupload.each(files, function(file) {
				previewImage(file,function(imgsrc){
					$('#uppic').prepend('<div class="uppic-content uppic-preview">\
							<img id="'+file.id+'" alt="pic" onload="common.size(this,100,100);" src="'
							+ imgsrc + '">\
							<span data-target="'+file.id+'" onclick="upPic.remove(this)" class="dis">&times;</span>\
						</div>'); 
                });
			});
		...
		UploadProgress: function(up, file) {
		    var progressBar = $('#uppic-progress-now');
		    var width = progressBar.width();
			progressBar.css('width',(file.percent+upconfig.count*100)/upconfig.filesNum+'%');
			progressBar.html((file.percent+upconfig.count*100)/upconfig.filesNum+'%');
		},

		FileUploaded: function(up, file, info) {
            if (info.status == 200)
            {
            	upconfig.fileList.push(upconfig.g_object_name);
            	upconfig.count++;
                if(upconfig.filesNum==upconfig.count){
                	upconfig.count=0;
                	upconfig.filesNum=0;
                	console.log(upconfig.fileList);
                	goods.issue(upconfig.fileList,function(data){
                		console.log(data);
                		setTimeout(function(){
                    		$('.tip-box').html('\
                        			<span id="logo" class="fa fa-check-circle success"></span>\
                        			<div id="new_goods_tip" class="tip-text">发布成功</div>\
                        			');
                        	$('#uppic-progress-now').removeClass('active');
                    	},500);
                		setTimeout(function(){
                    		$("#new").modal('hide');
                        	upPic.removeLoading();
...
upPic.checkPic = function(){
	...
	if(length>5){
		$('body').prepend('<div class="submit-shade">\
				<div class="tip-box">\
				<span id="logo" class="fa fa-times-circle failed"></span>\
				<div id="new_goods_tip" class="tip-text">最多上传5张图片</div>\
				</div>\
			</div>');
		setTimeout(function(){
	    	upPic.removeLoading();
	...
	for(;i<length;i++){
		uploader.removeFile(uploader.files[5]);
	}
};
